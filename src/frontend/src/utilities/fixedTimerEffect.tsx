// This is the fixed timer effect that solves the infinite loop issue

// Modified timer effect using useRef to track interval state across renders
const timerIntervalRef = useRef<NodeJS.Timeout | null>(null);
const allTracesCompleteRef = useRef(false);
const updateCountRef = useRef(0);
const MAX_UPDATES = 300; // Stop updating after ~5 minutes (~5 minutes)
        ...
        if (msg.type === 'trace-group') {
          ...
          if (!hasFinalResponse) {
            ...
            if (baseTitleParts.length > 0) {
              ...
              // Use the actual task count instead of relying on the regex match
              const regularTaskCount = 'tasks' in traceGroup && traceGroup.tasks ? 
                traceGroup.tasks.filter(t => t.stepNumber > 0).length : 0;
